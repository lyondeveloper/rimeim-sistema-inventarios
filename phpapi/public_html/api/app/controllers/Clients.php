...
class Clients extends Controller
{
    ...
    public function add()
    {
        ...
        $this->add_clients_products_prices(
            $new_id,
    ...
    private function validate_add_data($data)
    {
        ...
        $data->id_archivo = $new_image_id != null ?
            $new_image_id : (isset($data->imagen->id) ? $data->imagen->id : null);
        return $data;
    }
    ...
    private function add_clients_products_prices($client_id, $precios_productos)
    {
        ...
        foreach ($precios_productos as &$producto_precio) {
            if (
                isset($producto_precio->id_producto) &&
    ...
    public function add_multiple()
    {
        ...
        if (
            !isset($data->clientes) ||
            !is_array($data->clientes) ||
            !(count($data->clientes) > 0)
        ) {
            $this->response(['error' => "Clientes invalidos"], ERROR_PROCESS);
        }
        foreach ($data->clientes as $cliente) {
            $client = json_set_null_params_if_not_exists($cliente, ['codigo', 'correo', 'telefono', 'id_archivo', 'es_empresa']);
            if (is_null($client->es_empresa)) {
                $client->es_empresa = false;
            }
            $cliente->id_empleado = $this->get_current_employe_id();
            $cliente->id_local = $this->get_current_id_local();
            $this->clientModel->add($cliente);
        }
        $this->response();
    }
    ...
    public function update($id)
    {
        ...
        $this->update_client_products_prices(
            $id,
            isset($data->precios_productos) ?
                $data->precios_productos : []
        );
    ...
    public function delete($id)
    {
        ...
        $success = $this->clientModel->delete_by_id(
            $id,
    ...
    {
        $files = $this->fileModel->save_all_files(
            $this->fileupload,
