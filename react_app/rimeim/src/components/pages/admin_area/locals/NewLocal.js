...
class NewLocal extends Component {
    ...
    onSaveNewLocal = () => {
        const { nombre, codigo, color_hex, ubicacion, es_bodega, descripcion, empleados } = this.state
        const newLocalData = {
            nombre,
            codigo,
            color_hex,
    ...
    render() {
        const {
            ...
            ubicacion, es_bodega, descripcion,
            current_action, empleados,
            errors: {
                codigo_error,
                nombre_error
            } } = this.state
        ...
        return (
            <React.Fragment>
                <NavbarAdmin navtype={ADMIN_EDIT_LOCAL} obj={{ onSave: this.onSaveNewLocal }} />
                                                ...
                                                <tr key={empleado.id_usuario} className={`${!empleado.habilitado && ('grey lighten-2')}`}>
                                                    <td className="checkbox-td">
                                                        <label onClick={this.onChangeCheckboxEmploye.bind(this, empleado)}>
                                                            <input type="checkbox" className="filled-in" id={`check${empleado.id_usuario}`} />
                                                            <span></span>
                                                        </label>
                                                    </td>
